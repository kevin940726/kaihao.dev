...
After we have both a commit marked as `good` and a commit marked as `bad`, the `bisect` program will start automatically. The output will display something like below.
...
Now we are being checked out to the commit in the middle between our starting and ending point. All we have to do now is to determine if this commit can reproduce the bug we are trying the find. In this case we can just list the files in the directory and find whether `index.js` exists. Or run a simple command.
...
You can simply mark the commit without checking out to it. Useful for when starting the bisect
...
Going further, we can start the bisect in one line.

```sh
$ git bisect start 301fd10 HEAD -- # Start bisect with `301fd10` marked as `bad` and `HEAD` marked as `good`
```
