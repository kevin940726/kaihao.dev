...
接下來我們只要在每一頁定義我們想要預先在 server side 載入的 API，就一樣可以享有 SSR 的優點。這部分我實作了一個客製的 API 叫 `prefetchQueries`，它只會在 server side 執行，所做的也就只是去打需要的 API 並把結果 cache 起來後交給前端 dehydrate。這邊算是比較特殊的地方，但是個人目前使用起來也滿方便的，雖然說也不是完全沒有缺點，例如 TTFB 會相當高（詳見底下[效能](#performance)的段落），也許之後有比較多的經驗之後可以再多研究有沒有更好的方法。
...
用鍵盤瀏覽文章列表頁時，關閉 modal 後就必須重新找尋一次上次點擊的連結，Dcard clone 中在關閉 modal 後會將 focus 返回到該 modal 的連結上，同樣的也適用於點擊 modal 的下一篇／下一篇後。
