...
```jsx {4-10,14,22-24,30-32,35,41}
const connectV5 = mapStateToProps => WrappedComponent => props => {
  const store = React.useContext(Context);
  ...
  }, [store, propsRef, stateRef, forceUpdate]);
...
8. `<Todo>` will unmount, so it calls the unsubscribe function in the effect, remove it's listener callback from the listeners array in the `subStore`.
...
Interesting that we are running `mapStateToProps` potentially 2 times in the children component, one is inside the listener callback and the other is triggered by parent's re-rendering. The latter should happens before the former, but both the state and the props should be up-to-date and the same between each run. For further optimizing performance, we can memoize the `mapStateToProps` function so that it won't have to call itself twice in such cases.
