...
**This post is lucky enough to be featured on the [official Redux documentation](https://react-redux.js.org/api/hooks#stale-props-and-zombie-children)!**
...
```jsx {4-10,14-17,28-32,38-40,43,49}
const connect = mapStateToProps => WrappedComponent => props => {
  const store = React.useContext(Context);
...
This version just simply breaks. We suffer again from the same _stale props and zombie children_ problem we mentioned throughout the whole post. As always, we'll go through each step to see where and why it went wrong.
