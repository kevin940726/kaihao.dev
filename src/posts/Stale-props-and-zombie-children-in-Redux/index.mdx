...
Then, let's create a todo component, and wrap it with `connect` _(We are building the API using `connect` HOC only here. We'll talk about `useSelector` later)._
...
You might notice that this is simply just a function to create an _event emitter_, which is exactly right and it's just as simple as that. The next step is to write our new `connect` HOC and put our `mapStateToProps` inside the listener callbacks to bail out updates early.
...
7. The mapped state is not shallowly equal, so we return the new state in `setState` function callback to re-render the component with the latest mapped state. `<TodoList>` then renders an empty `<ul>` since there're no items in the list anymore.
8. `<Todo>` un-mounts, calls the unsubscribe function in the effect, remove it's listener callback from the listeners array in the `subStore`.
9. `<TodoList>` calls `subStore.notifyUpdates()` after the render, since we don't have any listeners in `subStore` left to call, the whole process completed successfully.

For cases where there are still some children left, Each children will then call their listener callbacks. Because they will be called after the render, they will have the latest props passed from their parents available.

Note that we don't even have to use `unstable_batchedUpdates` in the `notifyUpdates` function. The updates in the same hierarchy call are _split_ into different `subStore`, the children component will only call the listener callback when the parent has finished re-rendered, so there's no need to batch them together.
