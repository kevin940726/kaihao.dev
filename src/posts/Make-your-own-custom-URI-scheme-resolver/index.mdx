...
All you have to know is that in line 5, we are extracting the `path` we mentioned above and assign it to a variable `pathname`. In line 7, we are executing shell script and calling `open` built-in command to open our url in the default browser, and the url we're opening is `https://mycompany.atlassian.com/browse/[pathname]`.
